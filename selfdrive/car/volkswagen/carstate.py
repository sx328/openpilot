...
from openpilot.selfdrive.car.volkswagen.values import (
    DBC,
    CANBUS,
    PQ_CARS,
    NetworkLocation,
...
class CarState(CarStateBase):
    ...
    def update(self, pt_cp, cam_cp, ext_cp, trans_type):
        ...
        ret.steeringAngleDeg = (
            pt_cp.vl["LWI_01"]["LWI_Lenkradwinkel"]
            * (1, -1)[int(pt_cp.vl["LWI_01"]["LWI_VZ_Lenkradwinkel"])]
        )
        ret.steeringRateDeg = (
            pt_cp.vl["LWI_01"]["LWI_Lenkradw_Geschw"]
            * (1, -1)[int(pt_cp.vl["LWI_01"]["LWI_VZ_Lenkradw_Geschw"])]
        )
        ret.steeringTorque = (
            pt_cp.vl["LH_EPS_03"]["EPS_Lenkmoment"]
            * (1, -1)[int(pt_cp.vl["LH_EPS_03"]["EPS_VZ_Lenkmoment"])]
        )
        ret.steeringPressed = abs(ret.steeringTorque) > self.CCP.STEER_DRIVER_ALLOWANCE
        ret.yawRate = (
            pt_cp.vl["ESP_02"]["ESP_Gierrate"]
            * (1, -1)[int(pt_cp.vl["ESP_02"]["ESP_VZ_Gierrate"])]
            * CV.DEG_TO_RAD
        )

        # Verify EPS readiness to accept steering commands
        hca_status = self.CCP.hca_status_values.get(
            pt_cp.vl["LH_EPS_03"]["EPS_HCA_Status"]
        )
        ...
        ret.brake = (
            pt_cp.vl["ESP_05"]["ESP_Bremsdruck"] / 250.0
        )  # FIXME: this is pressure in Bar, not sure what OP expects
        brake_pedal_pressed = bool(pt_cp.vl["Motor_14"]["MO_Fahrer_bremst"])
        brake_pressure_detected = bool(pt_cp.vl["ESP_05"]["ESP_Fahrer_bremst"])
        ret.brakePressed = brake_pedal_pressed or brake_pressure_detected
        ret.parkingBrake = bool(
            pt_cp.vl["Kombi_01"]["KBI_Handbremse"]
        )  # FIXME: need to include an EPB check as well

        # Update gear and/or clutch position data.
        if trans_type == TransmissionType.automatic:
            ret.gearShifter = self.parse_gear_shifter(
                self.CCP.shifter_values.get(
                    pt_cp.vl["Getriebe_11"]["GE_Fahrstufe"], None
                )
            )
        elif trans_type == TransmissionType.direct:
            ret.gearShifter = self.parse_gear_shifter(
                self.CCP.shifter_values.get(
                    pt_cp.vl["EV_Gearshift"]["GearPosition"], None
                )
            )
        ...
        ret.doorOpen = any(
            [
                pt_cp.vl["Gateway_72"]["ZV_FT_offen"],
        ...
        if self.CP.enableBsm:
            ret.leftBlindspot = bool(
                ext_cp.vl["SWA_01"]["SWA_Infostufe_SWA_li"]
            ) or bool(ext_cp.vl["SWA_01"]["SWA_Warnung_SWA_li"])
            ret.rightBlindspot = bool(
                ext_cp.vl["SWA_01"]["SWA_Infostufe_SWA_re"]
            ) or bool(ext_cp.vl["SWA_01"]["SWA_Warnung_SWA_re"])
        ...
        ret.stockAeb = bool(ext_cp.vl["ACC_10"]["ANB_Teilbremsung_Freigabe"]) or bool(
            ext_cp.vl["ACC_10"]["ANB_Zielbremsung_Freigabe"]
        )

        # Update ACC radar status, modified for No-ACC VW
        self.acc_type = 0
        ...
        if self.CP.pcmCruise:
            ret.cruiseState.speed = (
                ext_cp.vl["ACC_02"]["ACC_Wunschgeschw_02"] * CV.KPH_TO_MS
            )
    ...
    def update_pq(self, pt_cp, cam_cp, ext_cp, trans_type):
        ...
        ret.vEgoRaw = (
            pt_cp.vl["Bremse_1"]["Geschwindigkeit_neu__Bremse_1_"] * CV.KPH_TO_MS
        )
        ret.vEgo, ret.aEgo = self.update_speed_kf(ret.vEgoRaw)
        ret.standstill = ret.vEgoRaw == 0
        ...
        # the sign/direction in a separate signal so they must be recombined.
        ret.steeringAngleDeg = (
            pt_cp.vl["Lenkhilfe_3"]["LH3_BLW"]
            * (1, -1)[int(pt_cp.vl["Lenkhilfe_3"]["LH3_BLWSign"])]
        )
        ret.steeringRateDeg = (
            pt_cp.vl["Lenkwinkel_1"]["Lenkradwinkel_Geschwindigkeit"]
            * (1, -1)[int(pt_cp.vl["Lenkwinkel_1"]["Lenkradwinkel_Geschwindigkeit_S"])]
        )
        ret.steeringTorque = (
            pt_cp.vl["Lenkhilfe_3"]["LH3_LM"]
            * (1, -1)[int(pt_cp.vl["Lenkhilfe_3"]["LH3_LMSign"])]
        )
        ret.steeringPressed = abs(ret.steeringTorque) > self.CCP.STEER_DRIVER_ALLOWANCE
        ret.yawRate = (
            pt_cp.vl["Bremse_5"]["Giergeschwindigkeit"]
            * (1, -1)[int(pt_cp.vl["Bremse_5"]["Vorzeichen_der_Giergeschwindigk"])]
            * CV.DEG_TO_RAD
        )

        # Verify EPS readiness to accept steering commands
        hca_status = self.CCP.hca_status_values.get(
            pt_cp.vl["Lenkhilfe_2"]["LH2_Sta_HCA"]
        )
        ret.steerFaultPermanent = hca_status in ("DISABLED", "FAULT")
        ret.steerFaultTemporary = hca_status in ("INITIALIZING", "REJECTED")
        ...
        ret.gasPressed = ret.gas > 0
        ret.brake = (
            pt_cp.vl["Bremse_5"]["Bremsdruck"] / 250.0
        )  # FIXME: this is pressure in Bar, not sure what OP expects
        ...
        if trans_type == TransmissionType.automatic:
            ret.gearShifter = self.parse_gear_shifter(
                self.CCP.shifter_values.get(
                    pt_cp.vl["Getriebe_1"]["Waehlhebelposition__Getriebe_1_"], None
                )
            )
        elif trans_type == TransmissionType.manual:
        ...
        ret.doorOpen = any(
            [
                pt_cp.vl["Gate_Komf_1"]["GK1_Fa_Tuerkont"],
        ...
        if self.CP.enableBsm:
            ret.leftBlindspot = bool(
                ext_cp.vl["SWA_1"]["SWA_Infostufe_SWA_li"]
            ) or bool(ext_cp.vl["SWA_1"]["SWA_Warnung_SWA_li"])
            ret.rightBlindspot = bool(
                ext_cp.vl["SWA_1"]["SWA_Infostufe_SWA_re"]
            ) or bool(ext_cp.vl["SWA_1"]["SWA_Warnung_SWA_re"])

        # Consume factory LDW data relevant for factory SWA (Lane Change Assist)
        # and capture it for forwarding to the blind spot radar controller
        self.ldw_stock_values = (
            cam_cp.vl["LDW_Status"]
            if self.CP.networkLocation == NetworkLocation.fwdCamera
            else {}
        )
        ...
        ret.cruiseState.speed = (
            ext_cp.vl["ACC_GRA_Anzeige"]["ACA_V_Wunsch"] * CV.KPH_TO_MS
        )
        ...
        ret.leftBlinker, ret.rightBlinker = self.update_blinker_from_stalk(
            300,
            pt_cp.vl["Gate_Komf_1"]["GK1_Blinker_li"],
    ...
    def get_can_parser(CP):
        ...
        messages = [
            ...
            (
                "Kombi_03",
                0,
            ),  # From J285 instrument cluster (not present on older cars, 1Hz when present)
        ]
